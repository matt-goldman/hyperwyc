...
public class TtlStalenessEvaluatorTests
{
    ...
    {
        var options = new RestycOptions { DefaultCacheTtl = TimeSpan.FromSeconds(10) };
...
public class ResponseCacheReadTests
{
    ...
    private static RestycHandler BuildHandler(
        InMemorySyncStore store,
        Fakes.StubHttpMessageHandler inner,
        bool cacheIsStale,
        int maxBodyBytes = 512 * 1024)
    {
        var options = new RestycOptions { MaxCachedResponseBodyBytes = maxBodyBytes };
