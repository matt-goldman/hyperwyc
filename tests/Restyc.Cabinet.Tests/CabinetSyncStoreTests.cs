using Restyc.Cabinet;
using Restyc.Models;
using Xunit;

namespace Restyc.Cabinet.Tests;
...
public class CabinetSyncStoreTests : IDisposable
{
    ...
    private static readonly byte[] TestKey =
        System.Security.Cryptography.SHA256.HashData("restyc-test"u8.ToArray());
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), $"restyc-tests-{Guid.NewGuid()}");
