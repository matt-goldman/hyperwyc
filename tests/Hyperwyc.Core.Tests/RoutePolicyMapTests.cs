...
public class RoutePolicyMapTests
{
    ...
    public void NoMatch_ReturnsTheDefault()
    {
        ...
        Assert.Equal(CacheStrategy.CacheFirst, Resolve(map, "https://example.com/other").Strategy);
    }
    ...
    public void ExactPattern_MatchesOnlyThatPath()
    {
        ...
        Assert.Equal(CacheStrategy.NetworkOnly, Resolve(map, "https://example.com/api/sales").Strategy);
        Assert.Equal(CacheStrategy.CacheFirst, Resolve(map, "https://example.com/api/sales/7").Strategy);
    }
    ...
    public void WildcardPattern_MatchesTheCollectionAndBeneath()
    {
        ...
        Assert.Equal(CacheStrategy.NetworkOnly, Resolve(map, "https://example.com/api/sales").Strategy);
        Assert.Equal(CacheStrategy.NetworkOnly, Resolve(map, "https://example.com/api/sales/7").Strategy);
        Assert.Equal(CacheStrategy.NetworkOnly, Resolve(map, "https://example.com/api/sales/7/lines").Strategy);

        // But not a sibling that merely shares a prefix string.
        Assert.Equal(CacheStrategy.CacheFirst, Resolve(map, "https://example.com/api/salesfigures").Strategy);
    }
    ...
    public void RegisteringTheSamePatternTwice_TheLaterOneApplies()
    {
        ...
        Assert.Equal(CacheStrategy.NetworkOnly, Resolve(map, "https://example.com/api/products").Strategy);
    }
    ...
    public void MatchingIgnoresSchemeHostAndPort()
    {
        ...
        Assert.Equal(CacheStrategy.NetworkOnly, Resolve(map, "http://localhost:5401/api/sales/7").Strategy);
        Assert.Equal(CacheStrategy.NetworkOnly, Resolve(map, "https://api.example.com/api/sales/7").Strategy);
    }
    ...
    public void MatchingIgnoresQueryString()
    {
        ...
        Assert.Equal(CacheStrategy.NetworkOnly, Resolve(map, "https://example.com/api/sales?page=2").Strategy);
    }
    ...
    public void NullUrl_ReturnsTheDefault()
    {
        ...
        // must not throw.
        Assert.Equal(CacheStrategy.NetworkOnly, map.Resolve(null).Strategy);
    }
    ...
    public void RecordSyntaxComposesOverAFactory()
    {
        ...
        Assert.Equal(CacheStrategy.CacheFirst, policy.Strategy);
    }
}
