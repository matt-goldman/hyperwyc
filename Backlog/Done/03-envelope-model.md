...
Define the `Envelope` document type that Restyc uses to persist both outbound (queued write) and inbound (cached response) data in the store.

## Background

Every request/response pair Restyc manages is stored as a single `Envelope` document. The same type covers both outbox entries (pending writes waiting for connectivity) and cache entries (GET responses stored for offline reads).
...
- [ ] `Envelope` and `CachedResponse` defined in `src/Restyc`.
