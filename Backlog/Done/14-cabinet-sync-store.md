...
- [x] `Restyc.Cabinet` project created (see issue #01); references `Restyc` and the Cabinet NuGet package.
- [x] `CabinetSyncStore : ISyncStore` implements all `ISyncStore` methods using Cabinet.
- [x] Cabinet indexes configured as shown above.
- [x] Database file path configurable via constructor: `new CabinetSyncStore("restyc.db")`.
- [x] `ResetAsync` deletes or truncates all Restyc-managed collections.
