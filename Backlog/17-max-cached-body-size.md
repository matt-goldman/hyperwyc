...
- If `body.Length > RestycOptions.MaxCachedResponseBodyBytes`:
...
- [ ] Limit is read from `RestycOptions.MaxCachedResponseBodyBytes`.
...
- In v0.1 the value is still read from `RestycOptions` (wired to the hardcoded default); this makes the v1.0 change a documentation/exposure update, not a code restructure.
