...
| 68 | [Cache invalidation leaves tombstones](68-cache-invalidation-leaves-tombstones.md) | ⬜ Open | `InvalidateCacheForPrefixAsync` nulls the response and keeps the envelope, leaving a record no query returns and nothing deletes. Reclaimed only if that exact URL is fetched again — and invalidation matches a *prefix*, so caching `/sales/1…500` then `POST /sales` orphans 500 records the app will never refetch. Costs more than disk: tombstones **retain the request headers** of the `GET` that made them ([30](30-sensitive-header-exclusion.md)), and every `UpdateAsync` in the loop is a full-store rewrite under [52](52-store-rewrites-whole-set-per-write.md), so invalidation is 52's worst case. The loop also does not filter to cache entries, needlessly rewriting queued writes under the same prefix — a two-line fix worth taking on its own |
