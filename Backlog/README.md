...
| 58 | [Docs describe behaviour the code no longer has](58-docs-code-reconciliation.md) | ⬜ Open | **Blocks first publish.** Eleven findings from a docs-vs-code pass on 2026-09-08, same shape as items 27–30. Worst: `events.md` documents a `DeliveryOutcomeKind.TransientFailure` that no longer exists, so the documented `switch` does not compile. Also: `delivery.md`'s reads table is wrong about offline TTL and about `NetworkOnly`, contradicting `responses.md`; the 2-second connectivity debounce it describes was removed by the ADR 0004 audit. Separately, `choosing.md`'s **Current limitations** lists one item where four open ones are consumer-visible and unworkaroundable — 41, 42, 43 and 53 |
| 61 | [XML documentation contradicts the code](61-xml-docs-contradict-the-code.md) | ⬜ Open | **Blocks first publish.** `HyperwycOptions.Routes` tells a consumer to register most specific first and says first-registered wins; `RoutePolicyMap` walks the list backwards, so following the XML silently applies the wrong policy. `ReplayTransport` still says replays bypass the pipeline, which item 37 and ADR 0002 reversed. Plus ADR 0004 leftovers in `OutboxProcessor` — a retry budget, a scheduled next attempt, a dangling backoff comment. This text ships in the package |
| 59 | [Events cannot be consumed without System.Reactive](59-events-without-system-reactive.md) | ⬜ Open | **Blocks first publish.** The primary example on `docs/events.md` uses `.Where` over `IObservable<T>`, which is `System.Reactive.Linq`. `HyperwycEventStream` is hand-rolled with no operators and the library takes no Rx dependency by design, so the documented snippet does not compile. Worst for a reader who followed the no-Rx advice in `connectivity.md`. Show the plain `IObserver` form |
| 56 | [Documentation restructure](56-documentation-restructure.md) | ⬜ Open | Every page interleaves instruction with explanation, so neither reader gets a path. `connectivity.md` is the worst case: 311 lines, third in **Start here**, and it opens by telling a MAUI reader they can skip it. Proposes a tutorial with next/previous links, per-host quick starts (MAUI first), reference pages stripped of essays, and explanation split into principles and patterns — where patterns is [50](50-resilient-applications-guide.md) finally being written. Collects the author's TODOs in `delivery.md` and `events.md`. Sequence after 58 |
...
| 62 | [Automatic store reset on failure](62-reset-store-on-failure.md) | 💭 Under consideration | **Recommendation: no.** `ResetStoreOnFailure = true` means silently discarding writes the caller was told were accepted — defaults test question 3 — and it fires in the case where the data is most likely intact, since the usual cause is a key or path change rather than corruption. It is also item 49's first draft coming back as an option. The smaller answer is a documented three-line subscribe-and-reset |
...
- **Items 56–63 came from the same exercise repeated on 2026-09-08**, ahead of the first publish,
  ...
  that ships inside the package ([61](61-xml-docs-contradict-the-code.md)). Two patterns are
