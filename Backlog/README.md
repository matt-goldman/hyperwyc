...
| 71 | [Benchmark the store, and the alternative implementations](71-store-benchmarks-and-alternative-implementations.md) | ⬜ Open | No numbers exist for the storage layer, and Hyperwyc's performance is currently Cabinet's performance. A harness measuring through `IHyperwycStore`, run over `InMemoryStore`, `CabinetStore` and LiteDB- and SQLite-backed stores built only as far as the comparison needs. **Deliberately one item**: whether the alternatives are worth finishing is what the benchmark answers, so splitting would plan the second half before its justification exists. Expected result is parity, which is the useful finding — it makes "good enough" evidenced rather than asserted, and moves the choice onto AOT, encryption and dependencies. Second value, independent of the numbers: a third and fourth implementation is the only real test that `IHyperwycStore` is buildable against — see [51](Done/51-cabinet-store-not-thread-safe.md). Expected to end as **reference code, not packages** ([ADR 0009](../docs/decisions/0009-provide-the-seam-not-the-alternatives.md)). Not a blocker |
