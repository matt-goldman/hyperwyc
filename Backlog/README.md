...
| 73 | [Review accessibility modifiers](73-review-accessibility-modifiers.md) | ⬜ Open | Audit every public type and member against [36](Done/36-public-surface.md)'s "start internal, widen on demand", since 22, 23, 49, 55, 62, 66 and 70 have all reshaped the surface since that pass. **1.0.0 is on NuGet, so every narrowing is technically a breaking change.** Scheduled here anyway because it is early days, and each release makes narrowing cost more. The strongest candidates come from constraints, not consumers: `StoreHealth` is public only because the handler's constructor takes it (36's event-stream trap, but with state-changing members), and `UsesDerivedEncryptionKey` is public only so the Cabinet package can set it |
