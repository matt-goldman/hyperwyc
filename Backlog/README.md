...
Items currently open in the current milestone. Completed milestone backlogs are listed below.

When commencing a new milestone, move the open items here and update the date and milestone.
...
| 73 | [Review accessibility modifiers](73-review-accessibility-modifiers.md) | ⬜ Open | Audit every public type and member against [36](Done/36-public-surface.md)'s "start internal, widen on demand", since 22, 23, 49, 55, 62, 66 and 70 have all reshaped the surface since that pass. **The urgent part is done**: `HyperwycEvent` and `PendingItem` were positional records, so adding a field broke the constructor and `Deconstruct` for everyone. They now use init-only properties. The test for urgent was *surface people use, in a shape we expect to change*, not *looks wrong*. What remains is longer-term: the full audit, a rule for how `IHyperwycStore` evolves (43 and 42 both plan to change it), how Core exposes things to the Cabinet package, and whether to enforce with `PublicApiAnalyzers`. The rest of the candidates, including `StoreHealth`, are housekeeping for whichever release needs a major bump anyway |
...
| 55 | [`Envelope.IsSynced` distinguishes two kinds by negation](Done/55-envelope-kind-discriminator.md) | ✅ Done | `Envelope` split into `QueuedWrite` (keyed by a generated id) and `CachedResponse` (keyed by the URL it caches), and `IHyperwycStore` into three cache methods, three outbox methods and `ResetAsync`. Option 2, as [66](66-dead-letter-store-fails-the-scope-test.md) predicted it would be once the flag's other two jobs were gone: splitting `UpsertAsync` was the entire cost, and the interface now states the kind split that four of its six methods were already living by. **Both stores now partition the kinds** — two dictionaries, two `RecordSet`s — so the `cache:` id prefix, the `!IsSynced` outbox filter and [68](Done/68-cache-invalidation-leaves-tombstones.md)'s `Response is not null` guard are all gone, the last two being kind checks written as field checks. **A cached response no longer stores the request it came from**, headers included, which was `Authorization` sitting in the cache for nothing. No ADR: this is ADR 0005's own finding being collected. **A preview store does not migrate** — it reads as empty rather than wrong, which is worse than [66](66-dead-letter-store-fails-the-scope-test.md)'s version of the same call |
...
| 20 | [Configurable body cache cap](Done/20-v1-configurable-body-cache-cap.md) | ✅ Done | `RoutePolicy.MaxCachedResponseBodyBytes` overrides the application-wide cap per route; unset inherits it. **The per-route override was the item, not a doc tidy** — the `UPDATE` line on the original said so, and it is right: one number for a whole API means raising the cap for the endpoint that returns a document raises it for every endpoint, on a store [42](42-cache-eviction.md) has not yet bounded. Inherited when unset, which is deliberately the opposite call to [29](Done/29-default-ttl-propagation.md)'s TTL — a validity bound has no safe fallback, a size cap has exactly one. `0` caches no bodies; **no unlimited sentinel**, because a body is a `byte[]` so `int.MaxValue` already exceeds anything cacheable, and `0`-for-unlimited would contradict `MaxOutcomeBodyBytes` two properties below it. Negatives throw at registration, naming the route. A suspected `Content-Length` bypass turned out not to exist — the body is buffered before the length is read — and is now pinned by a test |
| 30 | [Caller-set headers are persisted](Done/30-sensitive-header-exclusion.md) | ✅ Done | **Closed as documentation, which is what it had already been reversed to.** [storage.md](../docs/storage.md#what-ends-up-on-disk) gains "What ends up on disk": every header as Hyperwyc saw it is persisted, why fidelity requires it, and how long each record kind keeps it — a queued write only while outstanding ([66](66-dead-letter-store-fails-the-scope-test.md)), a cached response for the life of an entry nothing evicts. It sits directly below the encryption-key section, so exposure and mitigation are read together. [pipeline.md](../docs/pipeline.md) gains the ordering recommendation's second reason. **No deny-list, and none filed** — the opt-in form remains the right shape but waits for someone to ask; a backlog entry for it would be a commitment dressed as a record. **No `Security` page either**: everything it would carry already has a home, so it would duplicate three sections or gut two pages. Revisit when a third security-shaped topic has nowhere to live |
...
| 66 | [The dead-letter store fails the scope test](66-dead-letter-store-fails-the-scope-test.md) | ✅ Done | **Decided as B — retain nothing once delivered — and built; [ADR 0010](../docs/decisions/0010-delivery-ends-hyperwycs-interest.md) is `Accepted`.** `MoveToDeadLetterAsync`, `IsDeadLettered` and `OnFailed` are gone; `Succeeded`/`Rejected` collapse into `Delivered`; `MarkDeliveredAsync` becomes `RemoveDeliveredAsync` and deletes the record, request body and headers with it. **`FlushOnStartup` now defaults to `false`** — the event is the only report of a delivery, so a flush inside host startup would race a subscriber that attaches later and lose the outcome silently. The option stays and turning it on is still reasonable; what changed is which way round the choice sits when nobody has made it. The `409`-body cost is accepted rather than solved, and filed as [67](67-configurable-response-retention.md). The original argument follows. **Scope decision — needs an ADR.** Hyperwyc succeeds or fails at *delivery*; a `201` and a `409` are the same event from its side. Yet a non-`2xx` is retained indefinitely and a `2xx` discarded — an asymmetry only coherent if a non-`2xx` is a *Hyperwyc* failure, which it is not. Scope test Q4 draws the line: the **event** passes (only Hyperwyc can join this response to a caller who got a `202` and moved on), the **store** fails (what remains is an ordinary HTTP response the app can file under its own correlation id). Would remove `MoveToDeadLetterAsync`, `IsDeadLettered`, `OnFailed`, and dissolve [24](24-v1-dead-letter-management.md). Honest tension: a `409` body is unrecoverable by re-reading |
...
| 23 | [Diagnostics view](Done/23-v1-diagnostics-view.md) | ✅ Done | Read-only outbox queries on `IHyperwyc`. **Narrowed by [66](66-dead-letter-store-fails-the-scope-test.md), not dissolved** — the dead-letter half is gone with the store, and what is left is genuinely ours: the outbox passes scope-test Q4 by name. It is also the only place a transport failure, which publishes no event, can be seen, and since 66 that is the only outcome still written down |
| 62 | [Non-destructive reset when the store cannot be read](Done/62-reset-store-on-failure.md) | ✅ Done | **Option 1 — quarantine once.** An unreadable store is renamed into a `quarantine` directory inside the store directory and a clean one started, so caching and queueing resume from the next request instead of being off for the session. Nothing is deleted; the usual cause is a changed key or path, so the bytes are intact and merely unopenable. **The existence of the directory is the counter** — a second failure falls back to [49](Done/49-unreadable-store-recovery.md)'s report-and-step-aside, which is right on its own merits (twice is systemic, not incidental) and bounds storage at 2× with no sweep, no scheduler and no setting. Inside rather than beside the store directory so 48's one backup-exclusion path still covers it. `TryQuarantineAsync` defaults to `false`, so every other `IHyperwycStore` is unchanged. `CabinetStore.OpenQuarantined` closes the **key-derivation trap** — the orphan's ciphertext was written under the original path's key, and opening the directory naively looks exactly like the damage that caused the quarantine; both directions are tested. `ResetStoreAsync` still discards, quarantine included, and re-arms it. **The generation counter was the part not in the design**: two concurrent failures, the first recovers and the second — reporting history — would have latched the session off a moment later |
...
| 71 | [Benchmark the store, and build the alternatives](71-store-benchmarks-and-alternative-implementations.md) | 💭 Under consideration | No numbers for the storage layer, and no defensible answer to "why Cabinet and not the thing I already use". **Low priority, and not on anyone's critical path.** One caveat: [55](Done/55-envelope-kind-discriminator.md) and [66](66-dead-letter-store-fails-the-scope-test.md) have both now reshaped `IHyperwycStore`, so an implementation written against the pre-55 shape is out of date; the interface is settled again as of 2026-09-10 — the benchmarks themselves are unaffected. Not to be confused with the conformance suite, which is a different artefact recorded in [51](Done/51-cabinet-store-not-thread-safe.md) |
| 72 | [Should the event stream have a delivery guarantee?](72-events-as-an-outbox.md) | 💭 Under consideration | `HyperwycEventStream` is hot and publishes indiscriminately: no subscriber at the moment an event is raised means the event is gone. [ADR 0010](../docs/decisions/0010-delivery-ends-hyperwycs-interest.md) raised the stakes without changing the mechanism — the event is now the *only* report of a delivery, so a missed one is a lost outcome. Investigate treating events as an outbox in their own right, retained until taken. **Deliberately prescribes nothing**, and the first thing to settle is the tension with 0010: is an undelivered notification Hyperwyc's own outstanding work, or is retaining events just [67](67-configurable-response-retention.md) by another route, given that the event carries the response? May well be an [ADR 0009](../docs/decisions/0009-provide-the-seam-not-the-alternatives.md) answer — `IObservable<T>` is already a seam a durable subscriber can sit behind |
...
| 74 | [Blazor demo, using IndexedDB](74-blazor-demo-indexeddb.md) | 💭 Under consideration | A Blazor WebAssembly sample next to the MAUI one, backed by IndexedDB. Pulls in `Hyperwyc.IndexedDb` from the unfiled items below, which is the one second store [ADR 0009](../docs/decisions/0009-provide-the-seam-not-the-alternatives.md) says would ship. **A proof of concept, probably more demo than useful**: in a browser the real Service Worker exists, and this only serves an app that wants offline without push and wants to stay entirely in .NET. WebAssembly only, since Blazor Server loses its circuit when offline and has nothing to intercept. Would also be the first time any Blazor claim in this repo has been run rather than reasoned about |
...
- **Persisted-format changes are free, for now — but delete and reinstall the sample app.**
  ...
  its first read, which looks like an unrelated HTTP bug. **That is self-healing now**, since
  [62](Done/62-reset-store-on-failure.md): the old store is set aside and a clean one started, so a
  shape change costs the store's contents rather than the app. Reinstalling is still the cleaner
  move on a dev device — the quarantine is a one-shot, and burning it on a shape change nobody
  needed to keep leaves nothing for a real failure. Revisit the whole line the day the first
  package ships.
