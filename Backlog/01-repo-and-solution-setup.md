...
- [ ] Solution file `Restyc.sln` exists at the repo root.
- [ ] `src/Restyc/Restyc.csproj` — class library targeting `netstandard2.1` and/or `net10.0` (TBD).
- [ ] `src/Restyc.Cabinet/Restyc.Cabinet.csproj` — class library; project reference to `Restyc`.
- [ ] `tests/Restyc.Tests/Restyc.Tests.csproj` — xUnit test project; project reference to `Restyc`.
- [ ] `tests/Restyc.Cabinet.Tests/Restyc.Cabinet.Tests.csproj` — xUnit test project; project reference to `Restyc.Cabinet`.
- [ ] `.editorconfig` and `Directory.Build.props` for consistent build settings.
- [ ] `.gitignore` appropriate for a .NET solution.
- [ ] `README.md` at the repo root (can start as a copy of the design-doc version).
- [ ] CI pipeline (GitHub Actions) that restores, builds, and runs tests on push.
