...
Provide a read-only diagnostics surface so developers (and end users) can inspect the current state of the Restyc store: what's pending in the outbox and what's stuck in dead-letter.
...
Add the following to `IRestyc` (or a new `IRestycDiagnostics` interface):
...
Add a `DiagnosticsPage` to `Restyc.Sample/MauiApp` that:
...
- [ ] `GetPendingOutboxAsync()` and `GetDeadLetteredAsync()` added to `IRestyc` (or a diagnostics interface).
- [ ] Implemented by the concrete Restyc service, delegating to `ISyncStore`.
