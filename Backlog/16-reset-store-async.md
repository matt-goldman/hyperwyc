# Issue 16 — `IRestyc.ResetStoreAsync()` — Logout / Cache Clearing

## Summary

Implement `IRestyc.ResetStoreAsync()`, which clears all Restyc-managed state from the store — typically called on user logout or when a full cache wipe is needed.
...
4. This method does **not** stop the orchestrator or unsubscribe from connectivity events — Restyc continues operating normally after the reset, just with an empty store.

## Acceptance Criteria

- [ ] `ResetStoreAsync(CancellationToken ct = default)` implemented on the concrete `Restyc` service class that backs `IRestyc`.
